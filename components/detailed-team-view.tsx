...
type FPLFilter = "all" | "fpl" | "non-fpl";
...
export function DetailedTeamView() {
  ...
  const [fplFilter, setFplFilter] = useState<FPLFilter>("all");
  ...
  const filteredData = useMemo(() => {
    return detailedTeamMembers.filter((member) => {
      ...
      const matchesFpl =
        fplFilter === "all" ||
        (fplFilter === "fpl" && member.isFPL) ||
        (fplFilter === "non-fpl" && !member.isFPL);
      return matchesSearch && matchesRole && matchesRelease && matchesPod && matchesFpl;
    });
  }, [searchTerm, filterRole, filterRelease, filterPods, fplFilter]);

  const hasFilters =
    filterRole || filterRelease || filterPods.size > 0 || searchTerm || fplFilter !== "all";
              ...
              <div className="col-span-4">
                <div className="flex items-center gap-2">
                  ...
                  {member.isFPL && (
                    <Badge
                      variant="outline"
                      className="text-[10px] py-0 px-1.5 h-4 bg-blue-500/15 text-blue-300 border-blue-500/40 shrink-0"
                    >
                      FPL
                    </Badge>
                  )}
